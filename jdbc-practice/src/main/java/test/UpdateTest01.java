...
public class UpdateTest01 {
	...
	public static void main(String[] args) {
		boolean result = updateDepartment(2L, "전략팀");
	...
	private static boolean updateDepartment(long no, String name) {
		...
		try {
			...
			//4. SQL 실행
			String sql = "update dept set name='"+name+"' where no='"+no+"'";

			int count =  stmt.executeUpdate(sql); //select - executeQuery, others - executeUpdate
